...
pub struct MatchedItem {
    pub item: Arc<dyn for<'a> SkimItem<'a>>,
...
pub struct ItemPool {
    length: AtomicUsize,
    pool: SpinLock<Vec<Arc<dyn for<'a> SkimItem<'a>>>>,
    /// number of items that was `take`n
    taken: AtomicUsize,

    /// reverse first N lines as header
    reserved_items: SpinLock<Vec<Arc<dyn for<'a> SkimItem<'a>>>>,
...
impl ItemPool {
    ...
    pub fn append(&self, mut items: Vec<Arc<dyn for<'a> SkimItem<'a>>>) -> usize {
        let len = items.len();
    ...
    pub fn take(&self) -> ItemPoolGuard<Arc<dyn for<'a> SkimItem<'a>>> {
        let guard = self.pool.lock();
        let taken = self.taken.swap(guard.len(), Ordering::SeqCst);
        ItemPoolGuard { guard, start: taken }
    }

    pub fn reserved(&self) -> ItemPoolGuard<Arc<dyn for<'a> SkimItem<'a>>> {
