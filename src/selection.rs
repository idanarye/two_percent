...
pub struct Selection {
    ...
    selected: BTreeMap<ItemIndex, Arc<dyn for<'a> SkimItem<'a> + Send + Sync>>,
...
impl Selection {
    ...
    pub fn act_select_raw_item(
        &mut self,
        run_num: u32,
        item_index: u32,
        item: Arc<dyn for<'a> SkimItem<'a> + Send + Sync>,
    ) {
    ...
    pub fn get_selected_indices_and_items(&self) -> (Vec<usize>, Vec<Arc<dyn for<'a> SkimItem<'a> + Send + Sync>>) {
        // select the current one
        let select_cursor = !self.multi_selection || self.selected.is_empty();
        let mut selected: Vec<Arc<dyn for<'a> SkimItem<'a> + Send + Sync>> = self.selected.values().cloned().collect();
    ...
    pub fn get_current_item(&self) -> Option<Arc<dyn for<'a> SkimItem<'a> + Send + Sync>> {
