...
impl Model {
    pub fn new(rx: EventReceiver, tx: EventSender, reader: Reader, term: Arc<Term>, options: &SkimOptions) -> Self {
        ...
        let matcher = if let Some(engine_factory) = options.engine_factory.as_ref() {
            ...
            let fuzzy_engine_factory: Rc<dyn for<'d> MatchEngineFactory<'d>> =
                Rc::new(AndOrEngineFactory::new(Box::new(
    ...
    fn act_append_and_select(&mut self, env: &mut ModelEnv) {
        ...
        let item: Arc<dyn for<'a> SkimItem<'a>> = Arc::new(query);
    ...
    fn restart_matcher(&mut self) {
        ...
        if !processed {
            ...
            let new_items: Vec<Arc<dyn for<'a> SkimItem<'a>>> = self.reader_control.as_ref().map(|c| c.take()).unwrap();
